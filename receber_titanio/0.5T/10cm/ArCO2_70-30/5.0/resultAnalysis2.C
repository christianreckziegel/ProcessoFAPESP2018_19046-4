...
void resultAnalysis2(){
	...
	TString sEnergy = "5.0";
	TString strBase = "ArCO2_70-30_Result_"+sEnergy+"GeV_0.5T_10cm_";
	...
	int nbinsR = 36; //25 é o ideal para 4600 -> 18/5000
	...
	for(int j = 1; j < n_entries; j++){
		...
		if(eventNumb[j] == eventNumb[j-1]){
			...
			cout << "Error in event " << eventNumb[j] << endl;
			}   
			...
			//cout << "Fitting " << eventNumb[j] << ", Estimated energy=" << estimEnergy << "GeV\n";
			//clear TGraph pointer
			delete gr;
	...
	double incE = pc*B*1.6*0.01*3.*6.242*incR/estimEnergy;
	cout << "Usando o histograma do raio R, obtemos um valor para a energia de: " << endl;
