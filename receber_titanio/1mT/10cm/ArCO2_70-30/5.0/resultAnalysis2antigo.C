...
void resultAnalysis2(){
	...
	TH2D* hRadPoint = new TH2D("RadiusPerPoint", "Radius per #points of fit "+sEnergy+"GeV muon under 0.5T ArCO_{2} (70-30)", 
				...
				nbinsR, 0.0, 200000.0); //radius from 0 to 500
	...
	for(int j = 1; j < n_entries; j++){
		...
		if(eventNumb[j] == eventNumb[j-1]){
			...
			if (!ok) {
				...
				double chi2RedLine = (fStrLine->GetChisquare())/(fStrLine->GetNDF());
				cout << "The straight line fitted is:	y = " << par[0] << "*x + " << par[1] << endl << endl;
				cout << "#Chi^{2}_{red} = " << chi2RedLine << endl;*/
			}   
