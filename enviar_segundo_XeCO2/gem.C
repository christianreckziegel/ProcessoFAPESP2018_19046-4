...
int main(int argc, char * argv[]) {
  ...
  int iniAvalanche = index*5;
  int finalAvalanche = iniAvalanche+5;
  ...
  double metArest = 15.;
  std::string strMetArest = "15";
  
  std::string strGas = "XeCO2_90-10";
  
  //TApplication app("app", &argc, argv); //sem isso não plota
  //plottingEngine.SetDefaultStyle();
  //Garfield::randomEngine.Seed(2); //meu original
  //Garfield::randomEngine.Seed(1); //do Lucas
  ...
  gas->SetComposition("xe", 90., "co2", 10.);
  gas->SetTemperature(293.15);
  gas->SetPressure(760.); // 1 atm in [Torr]
  gas->EnableDrift(); // Allow for drifting in this medium
  gas->SetMaxElectronEnergy(10000.); //10 keV
  ...
  //TString fileName = "Result/"+strB+"T_"+strMetArest+"cm/"+strGas+"/"+strMuonEnergy+"/"+strGas+"_Result_"+strMuonEnergy+"GeV_"+strB+"T_"+strMetArest+"cm_"+strIndex+".root";
  ...
  for (int i = iniAvalanche; i < finalAvalanche; i++) {
    ...
    std::cout << "Inital position (x,y,z) = (" << xi << "," << yi << "," << zi << ")" << std::endl;
    ...
    //variables for uncertainty area calculation
    double xincMax = -500.;
    double xincMin = 500.;
    double yincMax = -500.;
    double yincMin = 500.;
    
    //variables for status collection
    int status1 = 0;
    int status3 = 0;
    int status5 = 0;
    int status7 = 0;
    ...
    for (int j=0; j<np; j++){
        ...
        //coleta número de ocorrência de cada status
        if(status == -1){ status1++; }
        if(status == -3){ status3++; }
        if(status == -5){ status5++; }
        if(status == -7){ status7++; }
        if(status == -8){ status8++; }
        if(status == -16){ status16++; }
        if(status == -17){ status17++; }
        //coleta os valores máximos e mínimos da posição em x e em y
        if(x1 > xincMax){ xincMax = x1; }
        if(x1 < xincMin){ xincMin = x1; }
        if(y1 > yincMax){ yincMax = y1; }
        if(y1 < yincMin){ yincMin = y1; }
    }
    cout << "In this avalanche...\n";
    cout << status1 << " particles left the drift area.\n";
    cout << status3 << " calculation abandoned (error, should not happen).\n";
    cout << status5 << " particles not inside a drift medium.\n";
    cout << status7 << " attachment.\n";
    cout << status8 << " sharp kink (only for RKF).\n";
    cout << status16 << " energy below transport cut.\n";
    cout << status17 << " outside the time window.\n";
