#!/bin/bash
#SBATCH --output=ArCH4_70-30_0.5T_15cm_2GeV_%A.%a.out
#SBATCH --job-name=ArCH4
#SBATCH --ntasks=1
#SBATCH --nodes=1
...
# Imprime o nó em que foi submetido o job
echo "Hostname = $SLURM_SUBMIT_HOST \n"

# Linha de execução do seu experimento
echo -e "\n## Submetendo job $SLURM_ARRAY_TASK_ID em $(date +'%d-%m-%Y as %T') #####################\n"
srun ./gem $SLURM_ARRAY_TASK_ID hostname
